...
class Storage:
    ...
    def save_top_songs_recommendation_to_bucket(self, data: dict, path: str):
        # Saving data to raw/landing storage
        logging.log(
            level=logging.INFO,
            msg=f"Saving json file to bucket [number of samples = {len(data)}]...",
        )
        filename = "file.json"
        ...
        # Create the schema of the raw json data in Trino so that we can query it later
